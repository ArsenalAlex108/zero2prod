...
impl<E: serde::de::Error> naan::semigroup::Semigroup
    for DeserializeError<E>
{
    fn append(self, b: Self) -> Self {
        E::custom(
            self.0.to_string() + "\n" + &b.0.to_string(),
        )
        .pipe(DeserializeError::from)
    }
}
...
pub struct DeserializeImpl2<T: Model<A, B>, A, B>(
    T,
    PhantomData<(A, B)>,
);
...
impl<
    'de,
    T: Model<A, B> + 'de,
    A: serde::Deserialize<'de> + Clone + 'de,
    B: serde::Deserialize<'de> + Clone + 'de,
> serde::Deserialize<'de> for DeserializeImpl2<T, A, B>
{
    fn deserialize<D>(
        deserializer: D,
    ) -> Result<Self, D::Error>
        ...
        D: serde::Deserializer<'de>,
    {
        struct Visitor<
            ...
            B: serde::Deserialize<'de> + Clone,
        >(PhantomData<&'de (T, A, B)>);
            ...
            for Visitor<'de, T, A, B>
        {
                ...
                V: serde::de::MapAccess<'de>,
            {
                ...
                Validation::from(Ok(T::new.curry()))
                    .apply(
                        a.map_err(DeserializeError::from)
                            .into(),
                    )
                    .apply(
                        b.map_err(DeserializeError::from)
                            .into(),
                    )
                    .pipe(Result::from)
                    .map_err(DeserializeError::inner)
                    .map(|i| {
                        DeserializeImpl2(i, PhantomData)
                    })
            }
        }
